...
std::atomic<double> total_duration_found{0.0};
std::atomic<double> total_duration_visited{0.0};



Crawler::Crawler(const std::string& start_url, const size_t num_threads, const std::string& file_path, size_t max_urls, std::chrono::seconds max_time, int cache_type ,bool lab_machine) :
...
void Crawler::start() {
    ...
    logger::info() << "\033[32mFound URLs: " << url_store.get_found_urls_size() << "\033[0m" << logger::endl;
    // logger::info() << "\033[32mAdd_visited_url time duration: " << total_duration_visited / 1000 << "ms\033[0m" << logger::endl;
    // logger::info() << "\033[32mAdd_found_url time duration: " <<  total_duration_found / 1000 << "ms\033[0m" << logger::endl;
    url_store.logging_operation_time();
    ...
    while(running){
        ...
        if(response.status_code == 200){
            ...
            if(response.content_type.find("text/html") != std::string::npos){
                ...
                total_duration_found = total_duration_found + duration.count();

                start = std::chrono::high_resolution_clock::now();
                url_store.add_visited_url(response.url);
                end = std::chrono::high_resolution_clock::now();
                duration = end - start;
                total_duration_visited = total_duration_visited + duration.count();
                ...
                start = std::chrono::high_resolution_clock::now();
                url_store.add_found_url_batch(new_links);
                end = std::chrono::high_resolution_clock::now();
                duration = end - start;
                total_duration_found = total_duration_found + duration.count();
            ...
            logger::debug() << "Redirecting to: " << response.redirect_url << logger::endl;
        } else {
            auto start = std::chrono::high_resolution_clock::now();
            url_store.add_visited_url(response.url);
            auto end = std::chrono::high_resolution_clock::now();
