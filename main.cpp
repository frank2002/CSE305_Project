...
int _main(int argc, char** argv) {
    ...
    logger::debug() << "cache_type: " <<  cache_type << logger::endl;
    Crawler crawler(url, threads, output, max_urls, max_time, cache_type ,lab_machine);
    crawler.start();
...
int test_thread() {
    initializeLogging();
    logger::DEFAULT_LOGGER = std::make_unique<logger::CustomLogger>(logger::Level::WARNING);
    ...
    for (int i = 1; i <= 64; i *= 2) {
        ...
        Crawler crawler(url, i, output, max_urls, max_time, cache_type ,lab_machine);
        crawler.start();
        ...
        logger::info() << "Sleep for 3 mins"<< logger::endl;
